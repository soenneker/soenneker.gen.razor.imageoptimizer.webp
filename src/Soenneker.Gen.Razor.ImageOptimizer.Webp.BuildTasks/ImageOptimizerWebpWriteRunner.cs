...
public sealed class ImageOptimizerWebpWriteRunner : IImageOptimizerWebpWriteRunner
{
    ...
    public async ValueTask<int> Run(string[] args, CancellationToken cancellationToken)
    {
        ...
        string? previousSettings = (await _fileUtil.Exists(settingsPath))
            ? await _fileUtil.Read(settingsPath, cancellationToken: cancellationToken)
            : null;
        ...
        if (result == 0 && failOnError && !string.Equals(settings, previousSettings, StringComparison.Ordinal))
        {
            ...
            await _fileUtil.Write(settingsPath, settings, cancellationToken: cancellationToken);
        }
        return result;
    }
